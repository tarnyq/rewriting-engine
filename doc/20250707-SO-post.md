...
typical strategy is to find any rule matches, apply it to produce a new
state, and carry on doing that until no rules match.)

We're at the state now where we have a performance problem that seems as if
it should be fixable, but we also don't know what we don't know about how
to better write these things in Haskell, so we're posting this to explain
...
The particular problem we're having right now is that GHC is not
...
in a real application, but that it has some branches and quite a bit of
...
matching for `decrement` (`$j_s2aS`) which here is repeating the entire
matching sequence for A and B to get to CI.

We don't see a huge performance penalty in this particular sample code
