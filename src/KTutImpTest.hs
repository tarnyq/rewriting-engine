...
tests = testGroup "KTutImp Tests"
  ...
  , testGroup "IO" [
      ...
      , testCase "SumToN Fail" $
        ...
        -- TODO: I'd link an integration test, something like:
        -- ./Test && echo -e '10' | ktutimp evalIO sumToNIO
  where stuck_read = [KI_AExp Read
                     , KI_Stmts ("n" := AHole)
                     , KI_Stmts ("sum" := Int 0)
                     , KI_Stmts (While (Not (Var "n" :<= Int 0)) (StmtsBlock (StPair ("sum" := (Var "sum" :+ Var "n")) ("n" := (Var "n" :+ Negate 1)))))
